...
async def view_rewards(ctx):
    ...
    if not rewards:
        await ctx.send("there are no rewards available/configured")
        return

    embed = discord.Embed(title="available rewards", color=discord.Color.blue())

    for reward_id, description, point_value, consumable in rewards:
        consumable_status = "y" if consumable else "n"
        embed.add_field(
            name=f"Reward ID: {reward_id}",
